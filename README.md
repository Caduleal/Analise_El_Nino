...
- **Taxa de Aquecimento Global:** Calculada entre +0,16°C e +0,17°C por década, em relação a uma média base de 18,30°C.
- **Assimetria de Massa Térmica (∫ ΔT dt):** Os dados atestam que os eventos de El Niño modernos acumulam exponencialmente mais energia latente na região do Pacífico Tropical (Niño 3.4) do que o resfriamento promovido pelas fases de La Niña.
- **Fenômeno da "Atenuação Fria":** A componente estritamente negativa das anomalias tem se suprimido a uma taxa de atenuação de +0,05°C por década.
...
2. **Média Espacial Ponderada (Correção Geométrica):**
   Para mitigar a super-representação das altas latitudes nas projeções espaciais, a média global aplica uma ponderação pelo cosseno da latitude (θ).
...
2. **Cálculo da Climatologia Base e Anomalia Espacial:**
   ...
   * Aplicação da ponderação pelo cosseno da latitude (cos(θ)) para extração correta das médias regionais e globais.
3. **Massa Térmica Acumulada e Estatística:**
   * Cálculo da integral da massa térmica acumulada (∫ ΔT dt) segregando fases quentes e frias.
